...
class SnakeGame:
    def __init__(self, seed=0, board_size=12, silent_mode=True, multi_process_render=False):
        self.board_size = board_size
        self.grid_size = self.board_size ** 2
        self.cell_size = 40
        self.width = self.height = self.board_size * self.cell_size
        self.score_space = self.cell_size
        self.border_size = 20
        self.display_width = self.width + 2 * self.border_size
        self.display_height = self.height + 2 * self.border_size + self.score_space
        self.silent_mode = silent_mode

        if not silent_mode or multi_process_render:
            if not silent_mode:
                pygame.init()
    ...
    def step(self, action):
        ...
        if (row, col) == self.food: # If snake eats food, it won't pop the last cell. The food grid will be taken by snake later, no need to update board vacancy matrix.
            ...
            food_obtained = False
            # Snake needs to shorten the tail to move forward. If food is eaten, tail shortening is skipped to grow.
    ...
    def _update_direction(self, action):
        if action == 0:
            ...
            if self.direction != "UP":
                self.direction = "DOWN"
        # Switch Case is supported in Python 3.10+
    ...
    def draw_score(self):
        """Draw score."""
        score_text = self.font.render(f"Score:{self.score:4}", True, (255, 255, 255))
